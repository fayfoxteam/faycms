...
class DeviceHelper{
    ...
    /**
     * 根据User Agent判断是否是蜘蛛访问
     *  - 若是蜘蛛，返回配置文件中定义的蜘蛛名称
     *  - 若非蜘蛛，返回空字符串
     * @param null|string $user_agent
     * @return string
     */
    public static function getSpider($user_agent = null){
        if($user_agent === null){
            $user_agent = Request::getUserAgent();
        }
        ...
        $spiders = \F::config()->getFile('spiders');
    ...
    /**
     * 根据User Agent判断是否为苹果移动设备（不包含mac，但包含ipad）
     * @param string $user_agent
     * @return bool
     */
    ...
    /**
     * 根据User Agent判断是否为windows phone
     * @param string $user_agent
     * @return bool
     */
    ...
    /**
     * 根据User Agent判断是否是IE浏览器（IE11也视为IE，因为有兼容模式，debug工具模拟什么的，无法确定到底是IE几）
     * @param string $user_agent
     * @return bool
     */
