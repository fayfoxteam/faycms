...
class FoodController extends FrontController{
	...
	public function index(){
		...
		if($this->form()->setRules(array(
			...
			if($cat_id = $this->form()->getData('cat_id', 0)){
				...
				$cat = Category::service()->getByAlias('food');
			}
			...
			$this->view->assign(array(
				'areas'=>$areas,
				'monthes'=>$monthes,
				'cats'=>Category::service()->getChildren('food'),
				'area_id'=>$area_id,
				'month_id'=>$month_id,
				'cat_id'=>$cat_id,
				'cat'=>$cat,
	...
	public function item(){
		...
		if(!$id || !$post = Post::service()->get($id, '', 'food', true)){
			throw new HttpException('页面不存在');
		}
		...
		$this->view->assign(array(
			'post'=>$post,
			'area'=>$area,
			'buy_link'=>Post::service()->getPropValueByAlias('food_buy_link', $id),
