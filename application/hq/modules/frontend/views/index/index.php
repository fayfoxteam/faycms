<?php 
$img_url = $this->staticFile('images');
?>
<div class="banner">
    <div id="indexbanner" class="indexbanner">

        <div class="bd">
            <ul>
                <li><a href="" target="_blank"><img src="<?= $img_url ?>/wl.jpg" /></a></li>
                
            </ul>
        </div>
    </div>
</div>
<div class="container">


<div class="index-min">

    <div class="index-mingsjj">
        <div class="index-mingsjjtit">图片新闻<span><a href=""><img src="<?= $img_url ?>/index_11.png" alt=""/></a></span> </div>
        <div class="clear-10"></div>
        <div class="clear-10"></div>
        <div class="hot_slider"> 
    <div class="slider_wrap"> 
        <div id="slider_box"> 
            <ul id="contentList"> 
                <li>
                    <a href="javascript:void(0)"><img src="<?= $this->staticFile('images/') ?>/320X260_1.jpg" alt="" /></a><div class="mask"></div>
                    <div class="comt"> 
                        <h3>高晓松酒驾被拘，达人秀评委可能当不成</h3>
                        <p>公众艺人并不好好遵纪守法，真让人失望呢！</p>
                    </div> 
                </li>
                <li>
                    <a href="javascript:void(0)"><img src="<?= $this->staticFile('images/') ?>/320X260_2.jpg" alt="" /></a><div class="mask"></div>
                    <div class="comt"> 
                        <h3>超可爱的小正太！那么小就会用眼勾人！</h3> 
                        <p>一起看看吧，值得一看，我被秒杀了！</p> 
                    </div> 
                </li>
                <li>
                    <a href="javascript:void(0)"><img src="<?= $this->staticFile('images/') ?>/320X260_3.jpg" alt="" /></a><div class="mask"></div>
                    <div class="comt"> 
                        <h3>陈奕迅超有爱连拍照！你绝对没有看见过</h3> 
                        <p>有没有被以上的帅锅美女给迷住呢？</p> 
                    </div> 
                </li>
                <li>
                    <a href="javascript:void(0)"><img src="<?= $this->staticFile('images/') ?>/320X260_4.jpg" alt="" /></a><div class="mask"></div> 
                    <div class="comt"> 
                        <h3>学校里的那些SB！你根本惹不起！</h3> 
                        <p>这些个图片大家看完后一定要淡定啊！</p>
                    </div> 
                </li>
            </ul> 
        </div>
    </div> 
</div>

    </div>
    <div class="index-minhxyw">
        <div class="index-mingsjjtit">后勤动态<span><a href=""><img src="<?= $img_url ?>/index_11.png" alt=""/></a></span> </div>
        <div class="clear-10"></div>
        <div class="index-minhxywmin">
            <ul>
                <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
            </ul>
        </div>
    </div>
    <div class="index-minxwzx ">
        <div class="index-mingsjjtit">最新公告
            <span>
                <a href="">
                    <img src="<?= $img_url ?>/index_11.png" alt=""/>
                </a>
            </span> 
        </div>
        <div class="clear-10"></div>
        <div class="clear-10"></div>

            <div class="index-minxwzxmin" id="gg" >
                <ul>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                    <li><i></i><a href="">新闻资讯新闻资讯新闻资讯新闻资讯</a> <span>[2015-4-26]</span></li>
                </ul>
            </div>
    

    </div>
</div>

<div id="serve">
    <div class="title">服务大厅</div>
    <div class="serve-list">
        <ul>
            <li><a href="">保修</a></li>
            <li><a href="">失物招领</a></li>
            <li><a href="">一卡通充值</a></li>
            <li><a href="">水电缴费查询</a></li>
            <li><a href="">网上订餐</a></li>
            <li><a href="">投诉建议</a></li>
        </ul>
    </div>
</div>
<div class="clear-20"></div>
<div id="content-bottom">
    <div class="left">
        <div class="guide">
            <div class="content-header">服务指南</div>
            <div class="content-box">
                <ul>
                    <li><a href="">餐饮服务</a></li>
                    <li><a href="">餐饮服务</a></li>
                    <li><a href="">餐饮服务</a></li>
                    <li><a href="">餐饮服务</a></li>
                    <li><a href="">餐饮服务</a></li>
                    <li><a href="">餐饮服务</a></li>
                    <li><a href="">餐饮服务</a></li>
                    <li><a href="">餐饮服务</a></li>
                    <li><a href="">餐饮服务</a></li>
                    <li><a href="">餐饮服务</a></li>
                    <li><a href="">餐饮服务</a></li>
                    <li><a href="">餐饮服务</a></li>
                </ul>
            </div>
            <div class="content-bottom">总值班电话：88888888（24小时）</div>
        </div>
    </div>

    <div class="center">
        <div class="content-header">快速通道</div>
        <div class="content-box">
            <div class="web web-1"><a href="">办公用品申领平台</a></div>
            <div class="web web-2"><a href="">水电使用监控平台</a></div>
            <div class="web web-3"><a href="">阳光厨房监控平台</a></div>
        </div>
    </div>

    <div class="right">
        <div class="hot-topic">
            <div class="content-header">热点专题</div>
            <div class="content-box"></div>
        </div>
    </div>
</div>

</div>
<script>
    $(function(){
        new Marquee("gg",0,1,345,200,100,0,1000,120,0);

        jQuery(".slider_wrap").slide({mainCell:"#slider_box ul",autoPlay:true});
    });

</script>