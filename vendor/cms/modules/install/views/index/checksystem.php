...
<table class="info-table">
	...
	<tr>
		...
		<td><?php foreach($writable as $k=>$w){?>
			<p><label class="w300" style="display:block;float:left;"><?php echo $k?></label><?php if($w){
				...
				$error_flag = true;
				echo '<span class="fc-red">不可写</span>';
			}?></p>
		<?php }?></td>
	</tr>
	<tr>
		...
		<td>
			<p>
				...
				<?php if(version_compare(PHP_VERSION, '5.2.0') == -1){
					$error_flag = true;
					echo '<span class="fc-red">PHP版本不能低于5.2.0</span>';
				}?>
			</p>
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			<p>
				<label class="w300" style="display:block;float:left;">mcrypt<span class="fc-grey">(不一定用到)</span></label>
				<?php if(in_array('mcrypt', $extensions)){
					echo '<span class="fc-green">支持</span>';
				}else{
					echo '<span class="fc-orange">不支持</span>';
				}?>
			</p>
			<p>
				<label class="w300" style="display:block;float:left;">memcache<span class="fc-grey">(不一定用到)</span></label>
