...
class FeedCommentService extends MultiTreeModel{
     ...
     */
    protected $model = 'fayfeed\models\tables\FeedCommentsTable';
     ...
     *  - parent.user.*系列可指定父评论作者信息，格式参照\cms\services\user\UserService::get()
     * @return array
     */
    public function get($comment_id, $fields = array(
        ...
        if(!empty($fields['user'])){
            $return['user'] = UserService::service()->get($comment['user_id'], $fields['user']);
        }
        ...
        if(!empty($fields['parent'])){
            ...
            if($parent_comment){
                ...
                if(!empty($fields['parent']['user'])){
                    $return['parent']['user'] = UserService::service()->get($parent_comment['user_id'], $fields['parent']['user']);
                }
