...
class WordController extends ApiController{
    ...
    public function get(){
        ...
        if(!$word['accept']){
            Response::notify(Response::NOTIFY_FAIL, '该用户未接受结义邀请，无法查看密语');
        }
        
        if($word['public_time'] > $this->current_time){
            Response::notify(Response::NOTIFY_FAIL, '离解密还有'.intval(($word['public_time'] - $this->current_time) / 86400).'天，请耐心等待。');
        }
        
        if(!GroupService::inGroup($word['group_id'], $this->current_user)){
            Response::notify(Response::NOTIFY_FAIL, '您不属于指定结义成员，无法查看密语');
        }
