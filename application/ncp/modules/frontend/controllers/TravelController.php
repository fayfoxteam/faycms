...
class TravelController extends FrontController{
	public function __construct(){
		...
		$this->layout->current_header_menu = 'travel';
	}
	...
	public function index(){
		...
		if($this->form()->setRules(array(
			...
			$this->view->assign(array(
				'areas'=>$areas,
				'monthes'=>$monthes,
				'cats'=>CategoryService::service()->getChildren('travel'),
				'area_id'=>$area_id,
				'month_id'=>$month_id,
				'cat_id'=>$cat_id,
				'cat'=>$cat,
	...
	public function item(){
		...
		$this->view->assign(array(
			...
			'food_posts'=>Recommend::model()->getByCatAndArea($food_cat, 9, OptionService::get('site:content_recommend_days'), $area['id']),
			'product_posts'=>Recommend::model()->getByCatAndArea($product_cat, 9, OptionService::get('site:content_recommend_days'), $area['id']),
			'right_posts'=>Recommend::model()->getByCatAndArea($product_cat, 6, OptionService::get('site:right_recommend_days')),
			'right_top_posts'=>Recommend::model()->getByCatAndArea($travel_cat, 2, OptionService::get('site:right_top_recommend_days'), 0, $id),
		))->render();
	}
}