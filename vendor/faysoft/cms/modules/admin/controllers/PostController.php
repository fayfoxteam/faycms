...
class PostController extends AdminController{
    ...
    public function create(){
        ...
        if($this->input->post() && $this->form()->check()){
            ...
            Response::notify(Response::NOTIFY_SUCCESS, '文章发布成功', array('cms/admin/post/edit', array(
                'id'=>$post_id,
            )));
        }
    ...
    public function edit(){
        ...
        if($this->input->post() && $this->form()->check()){
            ...
            Response::notify(Response::NOTIFY_SUCCESS, '一篇文章被编辑');
        }
    ...
    public function batch(){
        ...
        switch($action){
            case 'set-published':
                ...
                Response::notify(Response::NOTIFY_SUCCESS, count($affected_rows) . '篇文章被发布');
            break;
            case 'set-draft':
                foreach($ids as $id){
                    if(!PostService::checkEditPermission($id, PostsTable::STATUS_PUBLISHED)){
                ...
                Response::notify(Response::NOTIFY_SUCCESS, count($affected_rows) . '篇文章被标记为“草稿”');
            break;
            case 'set-pending':
                foreach($ids as $id){
                    if(!PostService::checkEditPermission($id, PostsTable::STATUS_PUBLISHED)){
                ...
                Response::notify(Response::NOTIFY_SUCCESS, count($affected_rows) . '篇文章被标记为“待审核”');
            break;
            case 'set-reviewed':
                foreach($ids as $id){
                    if(!PostService::checkEditPermission($id, PostsTable::STATUS_PUBLISHED)){
                ...
                Response::notify(Response::NOTIFY_SUCCESS, count($affected_rows) . '篇文章被标记为“通过审核”');
            break;
            case 'delete':
                foreach($ids as $id){
                ...
                Response::notify(Response::NOTIFY_SUCCESS, count($affected_rows) . '篇文章被移入回收站');
            break;
            case 'undelete':
                foreach($ids as $id){
                ...
                Response::notify(Response::NOTIFY_SUCCESS, count($affected_rows) . '篇文章被还原');
            break;
            case 'remove':
                foreach($ids as $id){
                ...
                Response::notify(Response::NOTIFY_SUCCESS, count($affected_rows) . '篇文章被永久删除');
            break;
            default:
