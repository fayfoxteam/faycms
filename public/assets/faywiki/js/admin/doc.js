var doc = {
    'docId': 0,
    'boxes': [],
    /**
     * 修改分类
     */
    'changeCategory': function(){
        $(document).on('change', '[name="cat_id"]', function(){
            //更新附加属性
            var $boxProps = $('#box-props');
            if($boxProps.length){
                $boxProps.block();
                $.ajax({
                    'type': 'GET',
                    'url': system.url('faywiki/admin/doc/get-prop-box', {
                        'cat_id': $(this).val(),
                        'doc_id': doc.docId
                    }),
                    'cache': false,
                    'success': function(resp){
                        $boxProps.unblock();
                        $boxProps.replaceWith(resp);
                    }
                });
            }
        });
    },
    /**
     * 弹出历史列表
     */
    'history': function(){
        //弹窗
        common.loadFancybox(function(){
            $('.show-doc-history-link').fancybox({
                'onComplete': function(){
                    doc.showHistoryList(0);
                }
            });
        });
        
        //绑定预览事件
        $(document).on('click', '.history-list li', function(){
            if($(this).attr('data-id')){
                $(this).addClass('crt').siblings().removeClass('crt');
                $('#history-preview').attr('src', system.url('cms/admin/doc-history/item', {
                    'history_id': $(this).attr('data-id')
                }));
            }
        }).on('click', '.history-list .load-more', function(){
            $(this).removeClass('load-more').addClass('loading').html('<div class="center">加载中...</div>');
            doc.showHistoryList($('#history-dialog').find('.history-list li[data-id]:last').attr('data-id'));
        });
    },
    /**
     * 获取并渲染历史记录列表（支持分页）
     * @param lastId 当前最后一条历史记录
     * @param pageSize
     */
    'showHistoryList': function(lastId, pageSize){
        if(!lastId){
            lastId = 0;
        }
        
        if(!pageSize){
            pageSize = 10;
        }

        var $historyDialog = $('#history-dialog');
        if(lastId == 0){
            //清空之前可能存在的列表
            $historyDialog.find('.history-list').html('');
        }
        $.ajax({
            'type': 'GET',
            'url': system.url('cms/admin/doc-history/list'),
            'data': {
                'last_id': lastId,
                'page_size': pageSize,
                'doc_id': doc.docId
            },
            'dataType': 'json',
            'cache': false,
            'success': function(resp){
                if(resp.status == 1){
                    //删除加载中描述
                    $historyDialog.find('.loading').remove();
                    
                    if(resp.data.histories.length){
                        $.each(resp.data.histories, function(i, data){
                            $historyDialog.find('.history-list').append([
                                '<li data-id="', data.id, '">',
                                    '<div class="time"><abbr>', system.date(data.create_time), '</abbr></div>',
                                    '<div class="user"><span>', data.user.user.nickname ? data.user.user.nickname : data.user.user.username, '</span></div>',
                                '</li>'
                            ].join(''))
                        });
                        if(lastId == 0){
                            //加载首页历史的时候，将第一条历史预览出来
                            doc.showFirstHistory();
                        }
                    }
                    if(resp.data.histories.length < pageSize){
                        $historyDialog.find('.history-list').append([
                            '<li>',
                                '<div class="center">没有了</div>',
                            '</li>'
                        ].join(''))
                    }else{
                        $historyDialog.find('.history-list').append([
                            '<li class="load-more">',
                                '<div class="center">加载更多</div>',
                            '</li>'
                        ].join(''))
                    }
                }else{
                    common.alert(resp.message);
                }
            }
        });
    },
    /**
     * 显示第一条历史
     * 若已经有历史记录被显示，则不做任何操作
     * 若没有历史纪录，则将iframe的src置空
     */
    'showFirstHistory': function(){
        var $historyList = $('#history-dialog').find('.history-list');
        if(!$historyList.find('li.crt').length){
            if($historyList.find('li[data-id]:first').length){
                $historyList.find('li[data-id]:first').click();
            }else{
                $('#history-preview').attr('src', '');
            }
        }
    },
    /**
     * 删除历史记录
     * @param id
     */
    'removeHistory': function(id){
        if(confirm('确定要删除当前历史记录吗？')){
            $.ajax({
                'type': 'POST',
                'url': system.url('cms/admin/doc-history/remove'),
                'data': {
                    'history_id': id
                },
                'dataType': 'json',
                'cache': false,
                'success': function(resp){
                    if(resp.status == 1){
                        $('#history-dialog').find('.history-list [data-id="'+id+'"]').addClass('bg-red').slideUp('slow', function(){
                            $(this).remove();
                            doc.showFirstHistory();
                            
                        });
                    }else{
                        common.alert(resp.message);
                    }
                }
            });
        }
    },
    /**
     * 回退至指定版本
     * @param id
     */
    'revertHistory': function(id){
        if(confirm('确定要文章回退至指定历史记录吗？')){
            window.location.href = system.url('cms/admin/doc-history/revert', {'history_id': id});
        }
    },
    'init': function(){
        if(system.inArray('category', doc.boxes)){
            this.changeCategory();
        }
        if(system.inArray('props', doc.boxes)){
            system.getScript(system.assets('faycms/js/admin/prop.js'), function(){
                prop.init();
            });
        }
        this.history();
    }
};